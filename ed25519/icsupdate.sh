...
IS_VALID_SIGNATURE=$?
	...
	if [ $IS_VALID_SIGNATURE -eq 1 ] ; then
		echo >&2 "incorrect GPG signature
		Check the signature file name for errors: ${ZIP_FILE_PATH}${ZIP_FILE}
		Aborting..."
		exit 1
	elif [ $IS_VALID_SIGNATURE -eq 2 ] ; then
        echo >&2 "invalid GPG signature h.
        Check the signature file name for errors: ${ZIP_FILE_PATH}${ZIP_FILE}
        Aborting..."
        exit 1 
    else
...
echo >&2 "pass .gpg files only as argument to the -f parameter"
exit 1
fi	
	echo "Checking signature..."
	...
	if [ $IS_VALID_SIGNATURE -eq 1 ] ; then
		echo >&2 "incorrect GPG signature
		Check the signature file name for errors: ${ZIP_FILE_PATH}${GPG_SIG_FILE}
		Aborting..."
		exit 1
	fi
	if [ $IS_VALID_SIGNATURE -eq 2 ] ; then
        echo >&2 "Invalid GPG signature.
        Check the signature file name for errors: ${ZIP_FILE_PATH}${GPG_SIG_FILE}
        Aborting..."
        exit 1 
    fi
...
if [ -f "${ZIP_FILE_PATH}${C_SIG_FILE}" ] ; then
	...
	${ZIP_FILE_PATH}Verify_file_arm ${ZIP_FILE_PATH}${ZIP_FILE} ${ZIP_FILE_PATH}${C_SIG_FILE}
	IS_VALID_SIGNATURE==$?
	
	if [ $IS_VALID_SIGNATURE -eq 1 ] ; then
		echo >&2 "Invalid signature for ${ZIP_FILE}.
		Check the signature file name for errors: ${ZIP_FILE_PATH}${C_SIG_FILE} 
		Aborting..."
		exit 1
	else
		echo "Correct signature, resuming update..."
